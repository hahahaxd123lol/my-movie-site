...
-- Existing installations may have this function with an older return type.
-- PostgreSQL cannot change a function return type with CREATE OR REPLACE,
-- so remove the exact signature first and recreate it below.
drop function if exists public.staff_get_profile_roles(uuid);
