...
function apply(state,{realtime=false}={}){
  const el=overlay();
  if(user?.id && state?.user_id && String(state.user_id)!==String(user.id)){
    el.classList.remove('show');el.hidden=true;document.documentElement.classList.remove('f2w-enforced');return;
  }
  const active=!!(state?.site_suspended||state?.account_banned);const uid=String(state?.user_id||user?.id||'');const key=JSON.stringify([uid,active,!!state?.site_suspended,!!state?.account_banned,state?.reason||'',state?.expires_at||'']);
...
async function boot(){boundedOps();window.__flix2watchAccountGuardReady=true;}
