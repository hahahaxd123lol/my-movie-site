(()=>{
'use strict';
if(window.__f2wV164Enforcement)return;window.__f2wV164Enforcement=true;
...
function enforcementCacheKey(uid){return `f2w_enforcement_v164:${String(uid||'')}`}
...
async function bindEnforcement(session){
  ...
  try{enforcementChannel=c.channel('f2w-v164-enforce-'+uid).on('postgres_changes',{event:'*',schema:'public',table:'account_enforcement_v146',filter:`user_id=eq.${uid}`},payload=>{
