...
function f2wDebounce(fn,wait=140){
  ...
  function addLiveSportsNav(){
    if(!document.getElementById('f2w-live-sports-nav-style')){
      const style=document.createElement('style');
      style.id='f2w-live-sports-nav-style';
      style.textContent=`
        .f2w-live-sports-link{gap:6px!important}
        .f2w-live-badge{display:inline-flex;align-items:center;gap:4px;padding:3px 6px;border-radius:999px;background:rgba(229,9,20,.16);border:1px solid rgba(255,44,55,.65);color:#ff4652;font-size:.49rem;font-weight:900;letter-spacing:.08em;line-height:1;text-transform:uppercase;box-shadow:0 0 0 rgba(229,9,20,.45);animation:f2wLivePulse 1.35s ease-in-out infinite}
        .f2w-live-dot{width:6px;height:6px;border-radius:50%;background:#ff2636;box-shadow:0 0 8px #ff2636;animation:f2wLiveDot 1.05s ease-in-out infinite}
        @keyframes f2wLivePulse{0%,100%{transform:scale(1);box-shadow:0 0 0 0 rgba(229,9,20,.34)}50%{transform:scale(1.05);box-shadow:0 0 0 5px rgba(229,9,20,0)}}
        @keyframes f2wLiveDot{0%,100%{opacity:.55;transform:scale(.78)}50%{opacity:1;transform:scale(1.2)}}
        @media (prefers-reduced-motion:reduce){.f2w-live-badge,.f2w-live-dot{animation:none!important}}
      `;
    ...
    document.querySelectorAll('.f2w-primary-nav').forEach(nav=>{
      ...
      if(location.pathname.startsWith('/live'))a.classList.add('active');
    });
  }
  ...
  async function installProfileEditor(){
    ...
    if(!authUser&&client){
      try{
        const {data}=await client.auth.getSession();
        authUser=data?.session?.user||authUser;
      }catch{}
    }
    ...
    if(button){
      ...
      button.hidden=!own;
      button.style.setProperty('display',own?'inline-flex':'none',own?'important':'');
      if(actions){
  ...
  window.f2wInstallProfileEditorV191=installProfileEditor;
        ...
        <div class="f2w-editor-scroll">
          <section class="f2w-editor-section active" data-section="identity">
            ...
            <div class="f2w-editor-grid">
              <div class="f2w-editor-field"><label>USERNAME</label><input id="v191-edit-username" maxlength="20" value="${esc(profile.username||'')}" placeholder="Username" autocomplete="off" spellcheck="false"><small>Your profile link will be /profile/@username. Letters and numbers only.</small></div>
  ...
  async function saveProfileEditor(){
    ...
    try{
      ...
      const previousUsername=String(viewedProfileObject()?.username||'');
      const requestedUsername=String(document.getElementById('v191-edit-username')?.value||'').trim();
      const result=await rpc('update_my_profile_v191',{
      ...
      try{
        if(typeof viewedProfile!=='undefined'&&viewedProfile){
          viewedProfile=Object.assign(viewedProfile,result||{});
          const nextUsername=String(viewedProfile.username||requestedUsername||previousUsername).replace(/[^A-Za-z0-9]/g,'');
          if(nextUsername){
            try{localStorage.setItem('f2w_profile_username_v24',nextUsername);}catch{}
            history.replaceState(null,'',`/profile/@${encodeURIComponent(nextUsername)}`);
          }
        }
      }catch{}
  ...
  async function boot(){
    forceRedLogo();addLiveSportsNav();addLeaderboardNav();hardenRouting();installDmSearch();reorderSourceButtons();
...
(() => {
  ...
  function paint(row){
    ...
    if(row.member_since){
      if(!window.__F2W_MEMBER_SINCE_V139)window.__F2W_MEMBER_SINCE_V139=row.member_since;
  ...
  window.addEventListener('f2w:profile-ready',()=>{
    setTimeout(()=>{installProfileEditor();renderProfileActivity();},0);
  });
})();
