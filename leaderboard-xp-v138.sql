-- Flix2Watch v138 — canonical XP-only leaderboard + accurate titles watched
-- Run after the previous migrations. Safe to re-run.

-- The leaderboard now has ONE canonical order: highest XP first.
-- p_sort is retained only for backwards compatibility with older deployed JS.
-- Titles watched is reconstructed from actual tracked watch-time rows plus legacy
-- title activity, so users with real watch time can no longer show 0 titles.
...
with title_keys as (
  select a.user_id,a.media_type,a.media_id
  from public.profile_title_activity a
  union
  select w.user_id,w.media_type,w.media_id
  ...
  -- Watch time remains the dominant source of XP.
  select b.*,
